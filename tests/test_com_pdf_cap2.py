...
def test_p0_prefix_hold_sem_embed():
    assets = PACOTE / "assets"
    html = _html()
    assert list(assets.glob("Cap2_P0_prefixB*")) == []
    assert list(assets.glob("Cap2_P0_prefixBD_*")) == []
    assert "Cap2_P0_prefixBD_" not in html
    assert "Cap2_P0_prefixB_" not in html
    assert "1EPi6wjWdL1lYai71eafFt8xQ7eD6NDqc" not in html
    assert "1jmhGbNjFbjrm5UpbaEZWXd1UwiLHEB7r" not in html
...
def test_p1_mito_correcao_seis_linhas():
    ...
    assert "Cap2_engine1962" not in html
    assert "M6_T_engineBayTin" not in html
    assert "Cap2_P0_prefixBD_" not in table
    prefixo = _slot(html, "Prefixo bloco B/BF/BH/BB/BD")
    ventoinha = _slot(html, "Ventoinha / correia")
    assert "Ausente" in prefixo
    assert "Ausente" in ventoinha
